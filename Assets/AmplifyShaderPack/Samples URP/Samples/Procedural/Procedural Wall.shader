	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_InnerAlbedo = IN.ase_texcoord9.xy * _InnerAlbedo_ST.xy + _InnerAlbedo_ST.zw;
				float2 uv_BrickAlbedo = IN.ase_texcoord9.xy * _BrickAlbedo_ST.xy + _BrickAlbedo_ST.zw;
				float2 uv_Noise = IN.ase_texcoord9.xy * _Noise_ST.xy + _Noise_ST.zw;
				float2 temp_output_27_0 = ( float2( 0.5,0.5 ) - ( _PatternSize * 0.5 ) );
				float2 temp_output_6_0 = ( _BrickTiling * IN.ase_texcoord9.xy );
				float temp_output_10_0 = (temp_output_6_0).y;
				float2 appendResult20 = (float2(( ( step( 1.0 , fmod( temp_output_10_0 , 2.0 ) ) * 0.5 ) + (temp_output_6_0).x ) , temp_output_10_0));
				float2 temp_output_17_0 = frac( appendResult20 );
				float2 smoothstepResult29 = smoothstep( temp_output_27_0 , ( temp_output_27_0 + _Slope ) , temp_output_17_0);
				...
				float2 uv_InnerNormal = IN.ase_texcoord9.xy * _InnerNormal_ST.xy + _InnerNormal_ST.zw;
				float2 uv_BrickNormal = IN.ase_texcoord9.xy * _BrickNormal_ST.xy + _BrickNormal_ST.zw;
...
ASEEND*/
//CHKSM=CDC5AE6D558B0C117DC7332D160B5CD3A231B048